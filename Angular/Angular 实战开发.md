*reference: [Course](https://www.bilibili.com/video/BV1dL4y1v7o9)*
# 1.  概述
...
在 Angular 应用中至少要有一个**根模块**，用于启动应用程序。
...
组件样式用来编写组件的组件的外观，组件样式可以采用 CSS、LESS、SCSS、Styles
...
在使用服务时不需要在组件类中通过 new 的方式创建服务实例对象获取服务中提供的方法，以下写法错误！
...
2. 创建共享组件：`ng g c shared/components/Layout` 可以指定路径
3. 在共享模块中导出共享组件 

   ```javascript
   @NgModule({
     declarations: [LayoutComponent],
     exports: [LayoutComponent] // 导出共享的组件
   })
...
在 Angular 中使用**插值表达式**进行数据绑定，即 {{ }} 大胡子语法。
...
<h2>{{a == b ? '相等': '不等'}}</h2>
<h2>{{'Hello Angular'}}</h2>
<p [innerHTML]="htmlSnippet"></p> <!-- 对数据中的HTML代码进行转义 -->
```
...
export class HomeComponent implements AfterViewInit {
  ...
  ngAfterViewInit() {
    console.log(this.items?.toArray()) // 使用 toArray方法获取元素数组
  }
}
```
...
> [!Summary] 导入全局样式文件的三种方式
> 1. 在 styles.css 中 import 样式表
> 2. 在 index.html 中导入样式文件cdn
> 3. 在 angular.json 文件中指定样式表
...
指令是 Angular 提供的**操作 DOM** 的途径。指令分为属性指令和结构指令。
...
export class FavoriteComponent {
  @Output() change = new EventEmitter() // 自定义 change 事件
  onClick() {
    this.change.emit({ name: "张三" }) // 抛出事件
  }
}
```
...
// 父组件类
export class AppComponent {
  onChange(event: { name: string }) { // 收到 change 事件的回调
    console.log(event)
  }
}
```
...
注入器负责**创建**服务类实例对象，并将服务类实例对象注入到需要的组件中。
...
1. 配置注入器的对象，指定了创建实例对象的服务类和访问服务实例对象的**标识**。
...
表单的控制逻辑写在**组件类**中，对验证逻辑拥有更多的控制权，适合复杂的表单的类型。
...
1. 在 Observable 对象内部可以多次调用 next 方法向外发送数据。(和Promise不一样)
